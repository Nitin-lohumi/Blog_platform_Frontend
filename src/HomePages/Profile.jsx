  ...
  const handleSaveImage = async (imageurl) => {
    try {
      ...
      const url = await axios.post(
        `http://13.60.32.184:3000/upload/Profile/${contextValue.ProfileData._id}`,
        { image: imageurl },
    ...
    const fetchUser = async () => {
      try {
        const res = await axios.get(
          `http://13.60.32.184:3000/posts/userProfile/${postId}`,
          { withCredentials: true }
        );
  ...
  return (
    <>
      <div className="Profile_Div">
        {postId === contextValue.ProfileData._id ? (
          <div>
      ...
      <div onClick={HandleOptionMenuClick} ref={Menu} className="MenuOption"><CiMenuKebab/></div>
       <div className="OptionMenu" style={{display:ClickMenu?"block":"none"}}>
        <div ref={ref2}><Logout/></div>
            ...
            <div className="UserInfo flex flex-col mt-2">
              <div className="flex flex-col p-2">
                <h1 className="text-center p-2 text-2xl font-bold">
                  {contextValue.ProfileData.name}
                </h1>
                <p className="text-center p-3">
                  <i className="font-thin text-wrap text-center">
                    {contextValue.ProfileData.email}
                  </i>
                </p>
              </div>
            </div>
            {/* Setting */}
            {NavigateProfile ? (
              <h1 className="font-bold text-xl mt-4 mb-6">YOUR POSTS</h1>
            ) : (
              ""
            )}
          ...
          <>
            {userProfile ? (
              <div className="UserInfo flex flex-col mt-2">
                ...
                <img
                  className="flex w-64 h-64 mt-10 mb-5 rounded m-auto"
